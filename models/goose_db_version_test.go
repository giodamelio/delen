...
var (
	gooseDBVersionDBTypes = map[string]string{`ID`: `INTEGER`, `VersionID`: `INTEGER`, `IsApplied`: `INTEGER`, `Tstamp`: `TIMESTAMP`}
	_                     = bytes.MinRead
)
...
func testGooseDBVersionsSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(gooseDBVersionAllColumns, gooseDBVersionPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, gooseDBVersionGeneratedColumns)
	}
